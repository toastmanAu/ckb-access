#!/usr/bin/env bash
# ckb full-node installer — Linux & macOS
...
echo "  ╚═══════════════════════════════════════════╝"
echo -e "${RESET}"
echo -e "  ${YELLOW}⚠  WARNING: Full mainnet sync requires 200GB+ of disk space.${RESET}"
echo "  Initial sync can take days depending on hardware."
echo "  Make sure your data directory has sufficient free space!"
echo ""

# ── Config ────────────────────────────────────────────────
write_step "Configuration"
ask INSTALL_DIR  "Install directory" "$HOME/.ckb-node"
ask RPC_PORT     "RPC port" "8114"
ask P2P_PORT     "P2P port" "8115"

DATA_DIR="${INSTALL_DIR}/data"
LOG_FILE="${DATA_DIR}/ckb.log"
...
if [ ! -f "${INSTALL_DIR}/ckb.toml" ]; then
  ...
  write_ok "Config generated by 'ckb init --chain mainnet'"
else
  write_ok "Config already exists — skipping init"
fi

# Patch RPC and P2P ports if non-default
...
if [ "$P2P_PORT" != "8115" ]; then
  if command -v sed &>/dev/null; then
    sed -i.bak "s|/ip4/0.0.0.0/tcp/8115|/ip4/0.0.0.0/tcp/${P2P_PORT}|g" \
      "${INSTALL_DIR}/ckb.toml" 2>/dev/null || true
...
if command -v ufw &>/dev/null && ufw status 2>/dev/null | grep -q active; then
  ...
  write_warn "No firewall detected — ensure port $P2P_PORT/tcp is open for P2P peers"
fi
...
write_info "Starting node briefly to verify it launches and RPC responds..."
...
echo ""
echo -e "  ${YELLOW}⚠  Full mainnet sync requires 200GB+ disk and may take several days.${RESET}"
echo ""
...
echo ""
write_ok "Done! Remember: mainnet sync needs 200GB+ disk and several days."
echo ""
